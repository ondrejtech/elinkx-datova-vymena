...
from importer import (
    insert_producers, insert_super_categories, insert_categories,update_category_images,
...
from automatization.productImport import main as import_products
...
def run():
    try:
        ...
        if(import_database == 'y'):
            ...
            log('Import obrazku kategorii')
            update_category_images(connection)
        ...
        # Import produktů z XML do DB
        import_products_choice = input('Do you want to import products from XML to DB? (y/n): ')
        if import_products_choice == 'y':
            log("📦 Spouštím import produktů z XML...")
            import_products()
